...
import { AppState, addToast, createEmptyProject, mergePrompt, newImageBox, newTextBox, persistProjects, persistUserPrefs, pushHistory, restoreSnapshot } from '../state';
import { AnyBox, Project, TextBox } from '../types';
...
export const PromptEditor: React.FC<Props> = ({ state, setState }) => {
  const project = state.projects.find(p => p.id === state.activeProjectId)!;
  const userPrefs = state.userPrefs || { pinnedTags: [] }; // defensive fallback
  ...
  // Helpers
  const updateProject = (mut: (p: Project) => void) => {
    setState(s => {
      pushHistory(s as any);
      const projects = s.projects.map(p => p.id === project.id ? { ...p, boxes: p.boxes.map(b => ({ ...b })) } : p);
      const target = projects.find(p => p.id === project.id)!;
      mut(target);
      // Enforce ordering: text boxes then image boxes
      const texts = target.boxes.filter(b => b.type === 'text');
      const images = target.boxes.filter(b => b.type === 'image');
      target.boxes = [...texts, ...images];
      target.modified = Date.now();
      return { ...s, projects };
    });
  };
  const setBox = (id: string, mut: (b: AnyBox) => void) => updateProject(p => { const b = p.boxes.find(b => b.id === id); if (b) { mut(b); (b as any).modified = Date.now(); } });
  const addText = (category: string) => { let newId: string | null = null; updateProject(p => { const b = newTextBox(category as any); newId = b.id; p.boxes.push(b); }); if (newId) setState(s => ({ ...s, lastFocusedBoxId: newId })); toast(`${category} box added`); };
  ...
  const duplicateBox = (id: string) => { updateProject(p => { const b = p.boxes.find(b => b.id === id); if (!b) return; const clone: AnyBox = { ...b, id: crypto.randomUUID(), created: Date.now(), modified: Date.now(), position: Date.now() } as AnyBox; p.boxes.push(clone); }); toast('Box duplicated'); };
  ...
  const imageBoxes = project.boxes.filter(b => b.type === 'image');

  // Tag data
  const tagFreq = useMemo(() => { const f: Record<string, number> = {}; project.boxes.forEach(b => { (b as any).tags?.forEach((t: string) => { f[t] = (f[t]||0)+1; }); }); return f; }, [project]);
  ...
  const quickAddTag = (t: string) => { if (state.lastFocusedBoxId) { updateProject(p => { const b = p.boxes.find(b => b.id === state.lastFocusedBoxId); if (b) { const arr = (b as any).tags || ((b as any).tags = []); if(!arr.includes(t)) arr.push(t); } }); toast(`Tag '${t}' added to box`); } else { updateProject(p => { if(!p.tags) p.tags=[]; if(!p.tags.includes(t)) p.tags.push(t); }); toast(`Tag '${t}' added to project`); } };
  ...
  const filteredImageBoxes = useMemo(() => { let list = imageBoxes; if (state.tagFilter.length) list = list.filter(b => passesTagFilter((b as any).tags||[])); return list; }, [imageBoxes, state.tagFilter, state.filterMode]);

  // Drag & drop
  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 6 } }));
  const [activeId, setActiveId] = useState<string | null>(null);
  const onDragEnd = (e: DragEndEvent) => { const { active, over } = e; setActiveId(null); if (!over || active.id === over.id) return; updateProject(p => { const group = p.boxes.filter(b => b.type === (active.data.current as any).boxType); const ids = group.map(b => b.id); const activeIndex = ids.indexOf(active.id as string); const overIndex = ids.indexOf(over.id as string); const newOrder = arrayMove(group, activeIndex, overIndex); const others = p.boxes.filter(b => b.type !== (active.data.current as any).boxType); p.boxes = (active.data.current as any).boxType === 'text' ? [...newOrder, ...others] : [...others, ...newOrder]; }); toast('Box order updated'); };
  ...
  const exportByTag = () => { if (!state.tagFilter.length) { toast('Select tag filters first'); return; } const selected = state.tagFilter; const matches = project.boxes.filter(b => { const tags = (b as any).tags||[]; if(!tags.length) return false; return state.filterMode === 'AND' ? selected.every(t => tags.includes(t)) : selected.some(t => tags.includes(t)); }).filter(b => b.type==='text') as TextBox[]; if (!matches.length) { toast('No boxes match tags'); return; } const lines: string[] = []; selected.forEach(tag => { const group = matches.filter(b => b.tags.includes(tag)); if (!group.length) return; lines.push(`## ${tag}`); group.forEach(b => { const w = b.weight>0? ` (w:${b.weight.toFixed(1)})` : ''; lines.push(`- ${b.content.trim()}${w}`); }); lines.push(''); }); const md = `# ${project.name} (Tag Export)\n\n${lines.join('\n')}`; const blob = new Blob([md], { type: 'text/markdown' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${project.name.replace(/\s+/g,'_')}_tags.md`; a.click(); toast('Tag export downloaded'); };
  const fullMarkdownExport = () => { const lines: string[] = []; lines.push(`# ${project.name}`); lines.push(`Created: ${new Date(project.created).toLocaleString()}`); lines.push(`Modified: ${new Date(project.modified).toLocaleString()}`); if(project.tags.length) lines.push(`Tags: ${project.tags.join(', ')}`); lines.push(''); const texts = textBoxes; if(texts.length) { lines.push('## Text Segments'); texts.forEach(b => { const tagStr = b.tags?.length ? ` [${b.tags.join(', ')}]` : ''; const weightStr = b.weight>0?` (w:${b.weight.toFixed(1)})`:''; lines.push(`- (${b.category})${weightStr}${tagStr}: ${b.content.trim()}`); }); lines.push(''); } if(imageBoxes.length){ lines.push('## Image References'); imageBoxes.forEach((b:any,i) => { const tagStr = b.tags?.length ? ` [${b.tags.join(', ')}]` : ''; lines.push(`- Image ${i+1}${tagStr}`); }); lines.push(''); } lines.push('## Merged Prompt'); lines.push('```'); lines.push(mergePrompt(project, state.activePreset)); lines.push('```'); const md = lines.join('\n'); const blob = new Blob([md], { type:'text/markdown' }); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`${project.name.replace(/\s+/g,'_')}.md`; a.click(); toast('Markdown exported'); };
  ...
  const deleteProject = (id: string) => { setState(s => { if (s.projects.length <= 1) { addToast(s, 'Cannot delete last project'); return { ...s }; } pushHistory(s as any); const remaining = s.projects.filter(p => p.id !== id); const activeProjectId = s.activeProjectId === id ? remaining[0].id : s.activeProjectId; addToast(s, 'Project deleted'); return { ...s, projects: remaining, activeProjectId }; }); };

  // Undo/Redo
  const undo = () => { setState(s => { const past: any[] = s.history.past; if (!past.length) { toast('Nothing to undo'); return { ...s }; } const last = past.pop(); const snapshot = restoreSnapshot(last); s.history.future.push({ data: JSON.stringify(s.projects), active: s.activeProjectId } as any); return { ...s, projects: snapshot, activeProjectId: last.active }; }); };
  const redo = () => { setState(s => { const fut: any[] = s.history.future; if (!fut.length) { toast('Nothing to redo'); return { ...s }; } const next = fut.pop(); s.history.past.push({ data: JSON.stringify(s.projects), active: s.activeProjectId } as any); const snapshot = restoreSnapshot(next); return { ...s, projects: snapshot, activeProjectId: next.active }; }); };
  ...
  return (<div className="app-shell">
    ...
    <div className="toolbar" role="banner">
      ...
      <select value={state.activePreset} onChange={e => setState(s => ({ ...s, activePreset: e.target.value as any }))} className="inline-input" style={{ width:140 }}>
          ...
          <div className="group">
            ...
            <DndContext sensors={sensors} onDragStart={e => setActiveId(String(e.active.id))} onDragEnd={onDragEnd}>
              <SortableContext items={filteredTextBoxes.map(b => b.id)} strategy={rectSortingStrategy}>
                <div className="boxes-grid">{filteredTextBoxes.map(b => <SortableBox key={b.id} box={b} setState={setState} onChange={(mut) => setBox(b.id, mut as any)} onDelete={() => deleteBox(b.id)} onDuplicate={() => duplicateBox(b.id)} toast={toast} invalid={b.category==='subject' && !b.content.trim()} />)}</div>
              </SortableContext>
              <h2>Images</h2>
...
const ToastHost: React.FC<{ toasts: AppState['toasts']; setState: React.Dispatch<React.SetStateAction<AppState>>; }> = ({ toasts, setState }) => { useEffect(() => { const t = setInterval(() => { const now = Date.now(); setState(s => ({ ...s, toasts: s.toasts.filter(t => now - t.ts < 3000) })); }, 1000); return () => clearInterval(t); }, [setState]); return <div className="toast-container" role="region" aria-live="polite" aria-label="Notifications">{toasts.map(t => <div key={t.id} className="toast" role="status">{t.message}</div>)}</div>; };
...
const SortableBox: React.FC<SortableBoxProps> = ({ box, onChange, onDelete, onDuplicate, toast, setState, invalid }) => { const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: box.id, data: { boxType: 'text' } }); const style: React.CSSProperties = { '--w': String((box.weight||0)/5), transform: CSS.Transform.toString(transform), transition } as any; return <div ref={setNodeRef} style={style} className={['prompt-box', isDragging && 'dragging', invalid && 'invalid'].filter(Boolean).join(' ')} data-weight={box.weight} aria-invalid={invalid || undefined}><div className="prompt-box-header"><span className={'badge category-'+box.category}>{box.category}</span><span className="drag-handle" {...attributes} {...listeners}>⠿</span><div className="weight-ctrl"><span className="material-symbols-rounded" style={{ fontSize:14, opacity:.6 }}>tune</span><IconButton icon="remove" tip="Dec Weight" onClick={() => onChange(b => { b.weight = Math.max(0, +(b.weight - 0.1).toFixed(1)); })} /><input value={box.weight} onChange={e => { const v = Math.min(5, Math.max(0, parseFloat(e.target.value)||0)); onChange(b => { b.weight = parseFloat(v.toFixed(1)); }); }} /><IconButton icon="add" tip="Inc Weight" onClick={() => onChange(b => { b.weight = Math.min(5, +(b.weight + 0.1).toFixed(1)); })} /></div></div><RichTextEditor boxId={box.id} value={box.richText || box.content} onFocus={() => setState(s => ({ ...s, lastFocusedBoxId: box.id }))} onChange={(html, plain) => onChange(b => { b.richText = html; b.content = plain; })} /><div className="box-actions"><IconButton icon="content_copy" tip="Copy" onClick={() => { navigator.clipboard.writeText(box.content); toast('Text copied'); }} /><IconButton icon="backspace" tip="Clear" onClick={() => { onChange(b => { b.content=''; b.richText=''; }); toast('Cleared'); }} /><IconButton icon="control_point_duplicate" tip="Duplicate" onClick={onDuplicate} /><IconButton icon="delete" className="danger" tip="Delete" onClick={onDelete} /></div><TagEditor tags={box.tags||[]} onAdd={t => onChange(b => { if(!b.tags.includes(t)) b.tags=[...b.tags,t]; })} onRemove={t => onChange(b => { b.tags = b.tags.filter(x => x!==t); })} />{invalid && <div style={{ fontSize:'.55rem', color:'#ff6b6b', letterSpacing:'.05em' }}>Subject is required</div>}</div>; };
...
const SortableImageBox: React.FC<SortableImageBoxProps> = ({ box, onChange, onDelete, onDuplicate, toast, setState }) => { const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: box.id, data: { boxType: 'image' } }); const style: React.CSSProperties = { transform: CSS.Transform.toString(transform), transition }; const onFile = async (file: File) => { const buf = await file.arrayBuffer(); const base64 = btoa(String.fromCharCode(...new Uint8Array(buf))); const dataUrl = `data:${file.type};base64,${base64}`; onChange(b => { b.content = dataUrl; (b as any).filename = file.name; }); toast(`Image "${file.name}" uploaded`); }; return <div ref={setNodeRef} style={style} className={['prompt-box', isDragging && 'dragging'].filter(Boolean).join(' ')}><div className="prompt-box-header"><span className="badge category-reference">image</span><span className="drag-handle" {...attributes} {...listeners}>⠿</span></div><div className="image-ref"><label className="thumb">{box.content ? <img src={box.content} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <input type="file" onChange={e => { const f=e.target.files?.[0]; if(f) onFile(f);} } />}</label><div style={{ flex:1, display:'flex', flexDirection:'column', gap:'.5rem' }}><input className="inline-input" placeholder="Description" value={(box as any).description||''} onFocus={() => setState(s => ({ ...s, lastFocusedBoxId: box.id }))} onChange={e => onChange(b => { (b as any).description = e.target.value; })} />{(box as any).filename && <div className="notice">{(box as any).filename}</div>}<div className="box-actions"><IconButton icon="content_copy" tip="Copy Data URL" onClick={() => { navigator.clipboard.writeText(box.content); toast('Image data copied'); }} /><IconButton icon="backspace" tip="Clear" onClick={() => { onChange(b => { b.content=''; (b as any).filename=''; }); toast('Image cleared'); }} /><IconButton icon="control_point_duplicate" tip="Duplicate" onClick={onDuplicate} /><IconButton icon="delete" className="danger" tip="Delete" onClick={onDelete} /></div><TagEditor tags={(box as any).tags||[]} onAdd={t => onChange(b => { const arr = (b as any).tags || ((b as any).tags=[]); if(!arr.includes(t)) (b as any).tags=[...arr,t]; })} onRemove={t => onChange(b => { (b as any).tags = ((b as any).tags||[]).filter((x: string)=>x!==t); })} /></div></div></div>; };

const IconButton: React.FC<{ icon: string; onClick?: (e: React.MouseEvent) => void; tip?: string; ariaLabel?: string; className?: string; children?: React.ReactNode; }> = ({ icon, onClick, tip, ariaLabel, className='', children }) => (<button type="button" aria-label={ariaLabel || tip || icon} data-tip={tip} onClick={onClick} className={['icon-btn', className].filter(Boolean).join(' ')}><span className="material-symbols-rounded" aria-hidden="true">{icon}</span>{children}</button>);
...
const TagEditor: React.FC<{ tags: string[]; onAdd: (t: string) => void; onRemove: (t: string) => void; inputRef?: React.RefObject<HTMLInputElement>; }> = ({ tags = [], onAdd, onRemove, inputRef }) => { const [value, setValue] = useState(''); const submit = () => { const t = value.trim().toLowerCase(); if (t && !tags.includes(t)) { onAdd(t); setValue(''); } }; const removeLast = () => { if (!value && tags.length) onRemove(tags[tags.length-1]); }; return <div className="tag-editor" role="group" aria-label="Tags"> <ul className="tag-list" style={{ listStyle:'none', padding:0, margin:0 }}>{tags.map(t => <li key={t}><span tabIndex={0} className="tag-chip" style={{ color: tagColor(t) }} onKeyDown={e => { if (e.key==='Delete' || e.key==='Backspace') { e.preventDefault(); onRemove(t); } }} >{t}<button aria-label={`Remove tag ${t}`} onClick={() => onRemove(t)}>×</button></span></li>)}</ul><div className="tag-add"><input ref={inputRef} placeholder="add tag" aria-label="Add tag" value={value} onChange={e => setValue(e.target.value)} onKeyDown={e => { if (e.key==='Enter') { e.preventDefault(); submit(); } else if (e.key==='Backspace') removeLast(); }} /><button className="icon-btn" data-tip="Add Tag" aria-label="Add tag" onClick={submit}><span className="material-symbols-rounded">add</span></button></div></div>; };

const RichTextEditor: React.FC<{ value: string; onChange: (html: string, plain: string) => void; onFocus?: () => void; boxId?: string; }> = ({ value, onChange, onFocus, boxId }) => { 
  ...
  const addLink = () => {
    let url = prompt('Enter URL'); if (!url) return; if (!/^https?:\/\//i.test(url)) url = 'https://' + url; exec('createLink', url);
  };
  ...
  return <div style={{ display:'flex', flexDirection:'column', gap:4 }}> 
    <div style={{ display:'flex', gap:4, flexWrap:'wrap', alignItems:'center' }}>
      <select aria-label="Heading level" onChange={e => { const v=e.target.value; if(v){ applyHeading(v); e.target.value=''; } }} defaultValue="" style={{ background:'var(--bg-elev-3)', color:'inherit', border:'1px solid var(--border)', borderRadius:4, fontSize:'.6rem', padding:'0 .25rem', height:28 }}>
        <option value="" disabled>Hdr</option>
...
const WeightLegend: React.FC = () => <div className="weight-legend" aria-label="Weight legend"><div className="label-row"><span>0</span><span>Light</span><span>Medium</span><span>Heavy</span><span>5</span></div><div className="bar" /></div>;