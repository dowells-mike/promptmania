...
import { AnyBox, Project, TextBox, ImageBox } from '../types';
...
export const PromptEditor: React.FC<Props> = ({ state, setState }) => {
  ...
  const updateProject = (mut: (p: Project) => void) => {
    setState(s => {
      ...
      mut(target);
      const texts = target.boxes.filter((b): b is TextBox => b.type === 'text');
      const images = target.boxes.filter((b): b is ImageBox => b.type === 'image');
  ...
  const filteredImageBoxes = useMemo(() => { let list = imageBoxes; if (state.tagFilter.length) list = list.filter(b => passesTagFilter((b as ImageBox).tags||[])); return list; }, [imageBoxes, state.tagFilter, state.filterMode]);
...
const SortableImageBox: React.FC<SortableImageBoxProps> = ({ box, onChange, onDelete, onDuplicate, toast, setState }) => { const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: box.id, data: { boxType: 'image' } }); const style: React.CSSProperties = { transform: CSS.Transform.toString(transform), transition }; const onFile = async (file: File) => { const buf = await file.arrayBuffer(); const base64 = btoa(String.fromCharCode(...new Uint8Array(buf))); const dataUrl = `data:${file.type};base64,${base64}`; onChange(b => { if(b.type==='image'){ b.content = dataUrl; (b as ImageBox).filename = file.name; } }); toast(`Image "${file.name}" uploaded`); }; return <div ref={setNodeRef} style={style} className={['prompt-box', isDragging && 'dragging'].filter(Boolean).join(' ')}><div className="prompt-box-header"><span className="badge category-reference">image</span><span className="drag-handle" {...attributes} {...listeners}>⠿</span></div><div className="image-ref"><label className="thumb">{box.content ? <img src={box.content} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <input type="file" onChange={e => { const f=e.target.files?.[0]; if(f) onFile(f);} } />}</label><div style={{ flex:1, display:'flex', flexDirection:'column', gap:'.5rem' }}><input className="inline-input" placeholder="Description" value={box.type==='image' ? (box as ImageBox).description||'' : ''} onFocus={() => setState(s => ({ ...s, lastFocusedBoxId: box.id }))} onChange={e => onChange(b => { if(b.type==='image'){ (b as ImageBox).description = e.target.value; } })} />{box.type==='image' && (box as ImageBox).filename && <div className="notice">{(box as ImageBox).filename}</div>}<div className="box-actions"><IconButton icon="content_copy" tip="Copy Data URL" onClick={() => { navigator.clipboard.writeText(box.content); toast('Image data copied'); }} /><IconButton icon="backspace" tip="Clear" onClick={() => { onChange(b => { if(b.type==='image'){ b.content=''; (b as ImageBox).filename=''; } }); toast('Image cleared'); }} /><IconButton icon="control_point_duplicate" tip="Duplicate" onClick={onDuplicate} /><IconButton icon="delete" className="danger" tip="Delete" onClick={onDelete} /></div><TagEditor tags={box.type==='image' ? (box as ImageBox).tags||[] : []} onAdd={t => onChange(b => { if(b.type==='image'){ const arr = (b as ImageBox).tags || ((b as ImageBox).tags=[]); if(!arr.includes(t)) (b as ImageBox).tags=[...arr,t]; } })} onRemove={t => onChange(b => { if(b.type==='image'){ (b as ImageBox).tags = ((b as ImageBox).tags||[]).filter(x=>x!==t); } })} /></div></div></div>; };
